  ...
  - name: send-to-valid-email
    executor:
      ...
      config:
        ...
        message: "This email was sent to a validated address."
    depends: validate-email
```
...
steps:
  - name: load-template
    command: |
      cat > /tmp/email_template.txt <<'EOF'
      Subject: ${SUBJECT}
    ...
    executor:
      ...
      config:
        ...
        message: |
          ...
          Dashboard: https://monitoring.company.com/${ENVIRONMENT}
```
...
steps:
  - name: compress-before-sending
    command: |
      ...
      if [ -f /tmp/large_report.csv ]; then
        gzip /tmp/large_report.csv
        echo "/tmp/large_report.csv.gz"
      fi
  ...
  - name: send-compressed
    executor:
      ...
      config:
        ...
        subject: "Compressed Report Attached"
        message: "Please find the compressed report attached."
        attachments:
          - ${COMPRESSED_FILE}
    depends: compress-before-sending
```
