...
### 1. Use Appropriate Data Passing Method

- **Small data** (< 1MB): Output variables
- **Large data**: File redirection
- **Structured data**: JSON with path references
...
steps:
  - name: get-json
    command: fetch-data.sh
    output: DATA
    
  - name: validate
    command: echo '${DATA}' | jq empty || exit 1
    depends: get-json
```
...
```yaml
# This workflow processes customer data
# Inputs:
#   - CUSTOMER_ID: Customer to process
# Outputs:
#   - REPORT_FILE: Generated report location
...
1. Check command produces output:
   ...
   steps:
     - name: debug
       command: |
         echo "Testing output" >&2  # Goes to stderr
         echo "Captured output"     # Goes to stdout
       output: TEST
   ```
...
2. Check variable is defined before use
