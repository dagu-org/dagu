...
- **Description**: Configuration for repeating steps.

#### Fields:
- **`repeat`** (boolean)
  - Enable unconditional repetition
...
- **`exitCode`** (array of integers)
  - Repeat on these exit codes

**Evaluation Logic** (priority order):

1. **With condition + expected**:
   - Repeat while `condition != expected`
   - Regex supported with `re:` prefix

2. **With condition only**:
   - Repeat while condition returns exit code 0

3. **With exitCode**:
   - Repeat if last exit code in list
...
# Unconditional repeat (daemon-like)
repeatPolicy:
  repeat: true
