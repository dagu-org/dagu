...
.PHONY: api
api: api-validate
	@echo -e "${COLOR_GREEN}Generating API...${COLOR_RESET}"
...
.PHONY: api-validate
api-validate:
	@echo -e "${COLOR_GREEN}Validating API...${COLOR_RESET}"
...
${LOCAL_DIR}/${PB_RELEASE_NAME}:
	@echo -e "${GREEN}Downloading protoc...${NC}"
...
build-image-version:
	...
	@echo -e "${COLOR_GREEN}Building the docker image with the version $(VERSION)...${COLOR_RESET}"
...
build-image-latest:
	@echo -e "${COLOR_GREEN}Building the docker image...${COLOR_RESET}"
...
.PHONY: addlicense
addlicense:
	@echo -e "${COLOR_GREEN}Adding license headers...${COLOR_RESET}"
...
.PHONY: bin
bin:
	@echo -e "${COLOR_GREEN}Building the binary...${COLOR_RESET}"
	@mkdir -p ${BIN_DIR}
...
build-keepalive:
	@echo -e "${COLOR_GREEN}Building keepalive binaries with Zig for all architectures...${COLOR_RESET}"
	...
	@echo -e "${COLOR_GREEN}Cleaning up build artifacts...${COLOR_RESET}"
	@rm -f internal/container/assets/keepalive_*.o internal/container/assets/keepalive_*.obj
	@echo -e "${COLOR_GREEN}Generating checksums...${COLOR_RESET}"
	...
	@echo -e "${COLOR_GREEN}Done! Checksums saved to internal/container/assets/keepalive_checksums.txt${COLOR_RESET}"
...
.PHONY: build-ui
build-ui:
	@echo -e "${COLOR_GREEN}Building UI...${COLOR_RESET}"
	@cd ui; \
		pnpm install; \
	...
	@echo -e "${COLOR_GREEN}Waiting for the build to finish...${COLOR_RESET}"
	@sleep 3 # wait for the build to finish

# build-ui-prod builds the frontend codes for production.
.PHONY: cp-assets
cp-assets:
	@echo -e "${COLOR_GREEN}Copying UI assets...${COLOR_RESET}"
	@rm -f ${FE_ASSETS_DIR}/*
...
.PHONY: clean-ui
clean-ui:
	@echo -e "${COLOR_GREEN}Cleaning UI build cache...${COLOR_RESET}"
	@cd ui; \
		rm -rf node_modules; \
		rm -rf .cache;
...
.PHONY: fmt
fmt:
	@echo -e "${COLOR_GREEN}Running go fix...${COLOR_RESET}"
	@go fix ./...
	@echo -e "${COLOR_GREEN}Running go fmt...${COLOR_RESET}"
	@go fmt ./...
	@echo -e "${COLOR_GREEN}Running linter with --fix...${COLOR_RESET}"
...
.PHONY: changelog
changelog:
	@echo -e "${COLOR_GREEN}Running changelog...${COLOR_RESET}"
...
${CA_CERT_FILE}:
	@echo -e "${COLOR_GREEN}Generating CA certificates...${COLOR_RESET}"
...
${SERVER_KEY_FILE}:
	@echo -e "${COLOR_GREEN}Generating server key...${COLOR_RESET}"
...
${SERVER_CERT_FILE}: ${CA_CERT_FILE} ${SERVER_KEY_FILE}
	@echo -e "${COLOR_GREEN}Generating server certificate...${COLOR_RESET}"
...
${CLIENT_KEY_FILE}:
	@echo -e "${COLOR_GREEN}Generating client key...${COLOR_RESET}"
...
${CLIENT_CERT_FILE}: ${CA_CERT_FILE} ${CLIENT_KEY_FILE}
	@echo -e "${COLOR_GREEN}Generating client certificate...${COLOR_RESET}"
	@openssl x509 -req -in ${CLIENT_CERT_REQ} -days 60 -CA ${CA_CERT_FILE} \
		-CAkey ${CA_KEY_FILE} -CAcreateserial -out ${CLIENT_CERT_FILE} \
		-extfile ${OPENSSL_CONF}

${CERTS_DIR}:
	@echo -e "${COLOR_GREEN}Creating the certificates directory...${COLOR_RESET}"
	@mkdir -p ${CERTS_DIR}

.PHONY: certs-check
certs-check:
	@echo -e "${COLOR_GREEN}Checking CA certificate...${COLOR_RESET}"
