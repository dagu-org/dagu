...
const testModelID = "automata-intg-model"

type testFixture struct {
	t             *testing.T
	th            test.Command
	automata      *automata.Service
...
func automataTestTimeout(timeout time.Duration) time.Duration {
	switch {
...
func newTestFixture(t *testing.T, dagName, dagYAML string, responses ...llmResponse) *testFixture {
	...
	require.NoError(t, modelStore.Create(th.Context, &agent.ModelConfig{
		ID:       testModelID,
		Name:     "Automata Integration Model",
		Provider: "local",
		Model:    "automata-intg",
	...
	automataService := automata.New(
		...
		automata.WithDAGRunController(&th.DAGRunMgr),
		automata.WithAgentAPI(agentAPI),
		automata.WithSubCmdBuilder(runtime.NewSubCmdBuilder(th.Config)),
	)
...
func (f *testFixture) putAutomata(name, dagName string) {
	f.t.Helper()
	require.NoError(f.t, f.automata.PutSpec(f.th.Context, name, `description: Integration test Automata
...
func (f *testFixture) startAutomata(name string) {
	f.t.Helper()
	_, err := f.automata.CreateTask(f.th.Context, name, automata.CreateTaskRequest{
		Description: "Run the integration test DAG",
		RequestedBy: "integration-test",
	})
	require.NoError(f.t, err)
	require.NoError(f.t, f.automata.RequestStart(f.th.Context, name, automata.StartRequest{
		RequestedBy: "integration-test",
		Instruction: "Run the allowlisted DAG now.",
	}))
	require.NoError(f.t, f.automata.ReconcileOnce(f.th.Context))
}

func (f *testFixture) waitForCurrentRun(name string, timeout time.Duration) exec.DAGRunRef {
	f.t.Helper()
	timeout = automataTestTimeout(timeout)
	...
	require.Eventually(f.t, func() bool {
		...
		detail, err := f.automata.Detail(f.th.Context, name)
		if err != nil {
			lastErr = err
			return false
		}
...
func (f *testFixture) startScheduler(timeout time.Duration) {
	...
	startupTimeout := automataTestTimeout(timeout)
	require.Eventually(f.t, func() bool {
		if err := f.pollSchedulerErr(); err != nil {
			return true
		}
...
func (f *testFixture) automataWorkspace(name string) string {
	return filepath.Join(f.th.Config.Paths.DataDir, "automata", name, "workspace")
}
...
func automataRunDAGArgs(dagName string) string {
