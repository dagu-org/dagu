...
const testModelID = "autopilot-intg-model"
...
func autopilotTestTimeout(timeout time.Duration) time.Duration {
	switch {
...
func newTestFixture(t *testing.T, dagName, dagYAML string, responses ...llmResponse) *testFixture {
	...
	require.NoError(t, modelStore.Create(th.Context, &agent.ModelConfig{
		ID:       testModelID,
		Name:     "Autopilot Integration Model",
		Provider: "local",
		Model:    "autopilot-intg",
	...
	autopilotService := autopilot.New(
		...
		autopilot.WithDAGRunController(&th.DAGRunMgr),
		autopilot.WithAgentAPI(agentAPI),
		autopilot.WithSubCmdBuilder(runtime.NewSubCmdBuilder(th.Config)),
	)
...
func (f *testFixture) putAutopilot(name, dagName string) {
	f.t.Helper()
	require.NoError(f.t, f.autopilot.PutSpec(f.th.Context, name, `description: Integration test Autopilot
...
func (f *testFixture) startAutopilot(name string) {
	f.t.Helper()
	_, err := f.autopilot.CreateTask(f.th.Context, name, autopilot.CreateTaskRequest{
		Description: "Run the integration test DAG",
		RequestedBy: "integration-test",
	})
	require.NoError(f.t, err)
	require.NoError(f.t, f.autopilot.RequestStart(f.th.Context, name, autopilot.StartRequest{
		RequestedBy: "integration-test",
		Instruction: "Run the allowlisted DAG now.",
	}))
	require.NoError(f.t, f.autopilot.ReconcileOnce(f.th.Context))
}

func (f *testFixture) waitForCurrentRun(name string, timeout time.Duration) exec.DAGRunRef {
	f.t.Helper()
	timeout = autopilotTestTimeout(timeout)
	...
	require.Eventually(f.t, func() bool {
		...
		detail, err := f.autopilot.Detail(f.th.Context, name)
		if err != nil {
			lastErr = err
			return false
		}
...
func (f *testFixture) startScheduler(timeout time.Duration) {
	...
	startupTimeout := autopilotTestTimeout(timeout)
	require.Eventually(f.t, func() bool {
		if err := f.pollSchedulerErr(); err != nil {
			return true
		}
...
func (f *testFixture) autopilotWorkspace(name string) string {
	return filepath.Join(f.th.Config.Paths.DataDir, "autopilot", name, "workspace")
}
...
func autopilotRunDAGArgs(dagName string) string {
