...
func (sc *Scheduler) Schedule(ctx context.Context, graph *ExecutionGraph, progressCh chan *Node) error {
	...
	NodesIteration:
		for _, node := range graph.nodes {
			if node.State().Status != status.NodeStatusNone || !isReady(ctx, graph, node) {
			...
			ExecRepeat: // repeat execution
				for setupSucceed && !sc.isCanceled() {
					...
					if node.State().Status != status.NodeStatusCancel {
						node.IncDoneCount()
					}
				...
				if node.State().Status == status.NodeStatusRunning {
					node.SetStatus(status.NodeStatusSuccess)
				}

				if err := sc.teardownNode(ctx, node); err != nil {
					sc.setLastError(err)
					node.SetStatus(status.NodeStatusError)
				}
	...
	case status.StatusNone, status.StatusRunning, status.StatusQueued:
...
func isReady(ctx context.Context, g *ExecutionGraph, node *Node) bool {
	...
	for _, dep := range g.To[node.id] {
		...
		case status.NodeStatusSkipped:
			...
			ready = false
			node.SetStatus(status.NodeStatusSkipped)
...
func (sc *Scheduler) isSucceed(g *ExecutionGraph) bool {
	...
	for _, node := range g.nodes {
		...
		if nodeStatus == status.NodeStatusSuccess || nodeStatus == status.NodeStatusSkipped || nodeStatus == status.NodeStatusPartialSuccess {
			continue
		}
		return false
	}
...
func (sc *Scheduler) isPartialSuccess(ctx context.Context, g *ExecutionGraph) bool {
	...
	for _, node := range g.nodes {
		if node.State().Status == status.NodeStatusError {
	...
	for _, node := range g.nodes {
		...
		case status.NodeStatusPartialSuccess:
			...
			hasFailuresWithContinueOn = true
			hasSuccessfulNodes = true
		case status.NodeStatusNone, status.NodeStatusRunning, status.NodeStatusCancel, status.NodeStatusSkipped:
...
func (sc *Scheduler) handleNodeExecutionError(ctx context.Context, graph *ExecutionGraph, node *Node, execErr error) (isRetriable bool) {
	...
	s := node.State().Status
	switch {
	case s == status.NodeStatusSuccess || s == status.NodeStatusCancel || s == status.NodeStatusPartialSuccess:
		// do nothing
...
func (sc *Scheduler) prepareNodeForRepeat(ctx context.Context, node *Node, progressCh chan *Node) {
	...
	node.SetStatus(status.NodeStatusRunning) // reset status to running for the repeat
