...
func (gh graphHelper) Schedule(t *testing.T, expectedStatus status.Status) scheduleResult {
	...
	ctx := digraph.SetupDAGContext(gh.Context, dag, nil, digraph.DAGRunRef{}, gh.Config.DAGRunID, logFilePath, nil, nil)
...
func TestScheduler_ErrorHandling(t *testing.T) {
	t.Run("SetupError", func(t *testing.T) {
		...
		ctx := digraph.SetupDAGContext(graph.Context, dag, nil, digraph.DAGRunRef{}, sc.Config.DAGRunID, logFilePath, nil, nil)
