...
func New(cfg *config.Config, dagStore exec.DAGStore, dagRunStore exec.DAGRunStore, opts ...Option) *Service {
	svc := &Service{
		cfg:                  cfg,
		definitionsDir:       filepath.Join(cfg.Paths.DAGsDir, "autopilot"),
		legacyDefinitionsDir: filepath.Join(cfg.Paths.DAGsDir, "automata"),
		stateDir:             filepath.Join(cfg.Paths.DataDir, "autopilot"),
		legacyStateDir:       filepath.Join(cfg.Paths.DataDir, "automata"),
		dagStore:             dagStore,
...
func validateName(name string) error {
	if !autopilotNamePattern.MatchString(name) {
		return fmt.Errorf("invalid autopilot name %q", name)
	}
	return nil
}
