...
func (s *Service) statePath(name string) string {
	newDir := filepath.Join(s.stateDir, name)
	if pathExists(newDir) {
		return filepath.Join(newDir, "state.json")
	}
	legacyDir := filepath.Join(s.legacyStateDir, name)
	if pathExists(legacyDir) {
		return filepath.Join(legacyDir, "state.json")
	}
	return filepath.Join(newDir, "state.json")
}

func (s *Service) autopilotWorkingDir(name string) (string, error) {
...
func (s *Service) ensureAutopilotWorkingDir(name string) (string, error) {
	dir, err := s.autopilotWorkingDir(name)
	if err != nil {
		return "", err
	}
	if err := os.MkdirAll(dir, dirPerm); err != nil {
		return "", fmt.Errorf("create autopilot workspace: %w", err)
	}
	return dir, nil
}
...
func (s *Service) resetRuntimeForSpecChange(ctx context.Context, name string) error {
	...
	if (state.State == StateRunning || state.State == StateWaiting) &&
		...
		queueTurnMessage(state, "config_updated",
			"Autopilot configuration changed. Continue with the latest configuration and current instruction.",
			s.clock(),
		)
...
func (s *Service) cleanupRuntime(ctx context.Context, name string, deleteSession bool) error {
	...
	if err := s.cancelAutopilotSession(ctx, name, state.SessionID); err != nil {
		return err
	}
...
func (s *Service) cancelAutopilotSession(ctx context.Context, name, sessionID string) error {
...
func (s *Service) reassignSessionUser(ctx context.Context, sessionID, newName string) error {
	...
	return errors.New("session store does not support autopilot rename with transcript preservation")
}

func (s *Service) assertAutopilotTargetAvailable(name string) error {
	if pathExists(filepath.Join(s.definitionsDir, name+".yaml")) || pathExists(filepath.Join(s.legacyDefinitionsDir, name+".yaml")) {
		return fmt.Errorf("autopilot %q already exists", name)
	}
	if pathExists(filepath.Join(s.stateDir, name)) || pathExists(filepath.Join(s.legacyStateDir, name)) {
		return fmt.Errorf("autopilot %q already has existing runtime state", name)
	}
	return nil
}
...
func (s *Service) systemUser(name string) agent.UserIdentity {
	return agent.UserIdentity{
		// Keep the legacy internal owner stable so upgraded installs can still
		// access existing persisted sessions that were created before the rename.
