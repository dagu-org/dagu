...
`))
	...
	require.Equal(t, eventstore.TypeAutopilotError, store.events[0].Type)
}

func TestRuntimeOptionsUsesAutopilotSoulDocument(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, "autopilot:software_dev", opts.Soul.ID)
