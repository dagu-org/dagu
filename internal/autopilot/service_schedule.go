...
func (s *Service) HandleScheduleTick(ctx context.Context, tickTime time.Time) error {
	...
	for _, def := range defs {
		if err := s.handleScheduledTick(ctx, def, tickTime); err != nil {
			s.logger.Warn("autopilot schedule tick failed",
				"autopilot", def.Name,
				"error", err,
			)
		}
...
func (s *Service) handleScheduledTick(ctx context.Context, def *Definition, tickTime time.Time) error {
	...
	if strings.TrimSpace(def.StandingInstruction) == "" {
		return s.recordScheduleConfigError(ctx, def, state, tickTime, "autopilot schedule requires a standing instruction")
	}
	if !hasTaskTemplates(state.TaskTemplates) {
		return s.recordScheduleConfigError(ctx, def, state, tickTime, "autopilot schedule requires at least one task template")
	}
