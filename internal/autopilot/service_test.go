...
func TestServiceListInitializesStateAndTaskSummary(t *testing.T) {
	...
	require.NoError(t, svc.PutSpec(ctx, "software_dev", autopilotSpec("build-app")))
...
func TestServiceDetailUsesTopLevelAllowedDAGs(t *testing.T) {
	...
	require.NoError(t, svc.PutSpec(ctx, "software_dev", autopilotSpecMultiDAGs()))
...
func TestServicePutSpecAcceptsLegacyPurposeAsGoalAlias(t *testing.T) {
	...
	spec := `description: Legacy autopilot
...
func TestServicePutSpecAcceptsMissingGoal(t *testing.T) {
	...
	spec := `description: Goal-free autopilot
allowed_dags:
  names:
    - build-app
`
	...
	require.Equal(t, "Goal-free autopilot", detail.Definition.Description)
}
...
func TestServicePutSpecRejectsDotsAndHyphensInAutopilotName(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	svc, _ := newTestService(t)

	err := svc.PutSpec(ctx, "software.dev", autopilotSpec("build-app"))
	require.ErrorContains(t, err, `invalid autopilot name "software.dev"`)
...
icon_url: https://cdn.example.com/autopilot/build-captain.png
...
`
	...
	require.Equal(t, "https://cdn.example.com/autopilot/build-captain.png", detail.Definition.IconURL)

	items, err := svc.List(ctx)
	require.NoError(t, err)
	require.Len(t, items, 1)
	require.Equal(t, "Build Captain", items[0].Nickname)
	require.Equal(t, "https://cdn.example.com/autopilot/build-captain.png", items[0].IconURL)
}
...
func TestServicePutSpecResetsRuntimeWhenAgentConfigChanges(t *testing.T) {
	...
	require.NoError(t, svc.PutSpec(ctx, "software_dev", autopilotSpecWithModel("build-app", "model-a")))
	...
	require.NoError(t, svc.PutSpec(ctx, "software_dev", autopilotSpecWithModel("build-app", "model-b")))
...
func TestServiceRequestStartRequiresInstructionAndOpenTask(t *testing.T) {
	...
	require.ErrorContains(t, err, "instruction is required before starting autopilot")
}
...
func TestServiceRequestStartAcceptsLegacyServiceSpec(t *testing.T) {
	...
	require.NoError(t, svc.PutSpec(ctx, "queue_worker", serviceAutopilotSpec("build-app")))
	...
	require.NoError(t, err)
	require.Equal(t, AutopilotKindWorkflow, detail.Definition.Kind)
	require.Equal(t, StateRunning, detail.State.State)
	...
	require.NoError(t, err)
	require.Len(t, items, 1)
	require.Equal(t, AutopilotKindWorkflow, items[0].Kind)
...
func TestServiceRequestStartRejectsSecondActiveCycle(t *testing.T) {
	...
	require.ErrorContains(t, err, "autopilot already has an active task")
}
...
func TestServiceRenameMovesMemory(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, memory.Content, "Remember the standing approach.")
	require.Contains(t, memory.Path, "/autopilot/software_ops/MEMORY.md")
	soul, err := svc.GetDocument(ctx, "software_ops", DocumentSoul)
	require.NoError(t, err)
	require.Contains(t, soul.Content, "Be precise.")
	require.Contains(t, soul.Path, "/autopilot/software_ops/SOUL.md")
...
func TestServiceDeleteRemovesMemory(t *testing.T) {
	...
	if svc.memoryStore != nil {
		content, loadErr := svc.memoryStore.LoadAutopilotMemory(ctx, "software_dev")
		require.NoError(t, loadErr)
		require.Empty(t, content)
		store, ok := svc.memoryStore.(agent.AutopilotDocumentStore)
		require.True(t, ok)
		soul, loadErr := store.LoadAutopilotDocument(ctx, "software_dev", DocumentSoul)
		require.NoError(t, loadErr)
...
func TestServicePauseRejectsIdleAutopilot(t *testing.T) {
	t.Parallel()
	...
	require.ErrorContains(t, err, "only active autopilot can be paused")
...
func TestServiceReconcileOnceDoesNotWakeIdleScheduledAutopilot(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	svc, fixedTime := newTestService(t)

	require.NoError(t, svc.PutSpec(ctx, "software_dev", autopilotSpecWithSchedule("build-app", "* * * * *")))
	createTask(t, svc, ctx, "software_dev", "Investigate the failing test", "tester")
...
func TestServiceHandleScheduleTickQueuesTurnForActiveService(t *testing.T) {
	...
	require.NoError(t, svc.PutSpec(ctx, "queue_worker", serviceAutopilotSpecWithSchedule("build-app", "* * * * *")))
	createTask(t, svc, ctx, "queue_worker", "Process the next queued request", "tester")
	require.NoError(t, svc.RequestStart(ctx, "queue_worker", StartRequest{
		RequestedBy: "tester",
		Instruction: "Handle inbound work continuously.",
...
description: Software development autopilot
goal: Complete the assigned software work
...
func TestServiceReconcileOnceReturnsInactiveRunningAutopilotToIdle(t *testing.T) {
	t.Parallel()
...
func TestControllerRuntimeRunAllowedDAGRejectsConcurrentRun(t *testing.T) {
	...
	rt := &controllerRuntime{service: svc, def: def, state: state}
	_, err = rt.RunAllowedDAG(ctx, agent.AutopilotRunDAGInput{DAGName: "build-app"})
...
func TestControllerRuntimeRequestHumanInputEmitsEvent(t *testing.T) {
	...
	rt := &controllerRuntime{service: svc, def: def, state: state}
	require.NoError(t, rt.RequestHumanInput(ctx, agent.AutopilotHumanPrompt{
		Question: "Approve the deployment?",
	}))

	require.Len(t, store.events, 1)
	event := store.events[0]
	require.Equal(t, eventstore.KindAutopilot, event.Kind)
	require.Equal(t, eventstore.TypeAutopilotNeedsInput, event.Type)
	require.Equal(t, "software_dev", event.AutopilotName)
	require.Equal(t, fixedTime, event.OccurredAt)
	snapshot, err := eventstore.NotificationAutopilotFromEvent(event)
	require.NoError(t, err)
...
func TestControllerRuntimeFinishAllowsLegacyServiceAutopilot(t *testing.T) {
	t.Parallel()
...
func TestControllerRuntimeFinishEmitsEventAndAssignsCycleID(t *testing.T) {
	...
	require.Len(t, store.events, 1)
	event := store.events[0]
	require.Equal(t, eventstore.TypeAutopilotFinished, event.Type)
	require.Equal(t, state.CurrentCycleID, event.AutopilotCycleID)
	require.Equal(t, fixedTime, event.OccurredAt)
	snapshot, err := eventstore.NotificationAutopilotFromEvent(event)
	require.NoError(t, err)
...
func TestControllerRuntimeFinishResetsWhenConfigured(t *testing.T) {
	...
	spec := autopilotSpec("build-app") + "reset_on_finish: true\n"
	require.NoError(t, svc.PutSpec(ctx, "software_dev", spec))
	createTask(t, svc, ctx, "software_dev", "Investigate the failing test", "tester")
	require.NoError(t, svc.RequestStart(ctx, "software_dev", StartRequest{
		RequestedBy: "tester",
	...
	event := store.events[0]
	require.Equal(t, eventstore.TypeAutopilotFinished, event.Type)
	require.Equal(t, cycleID, event.AutopilotCycleID)
	snapshot, err := eventstore.NotificationAutopilotFromEvent(event)
	require.NoError(t, err)
...
func TestServiceRuntimeOptionsIncludeFinishToolForLegacyService(t *testing.T) {
	...
	opts, err := svc.runtimeOptions(ctx, def, state)
	require.NoError(t, err)
	require.Contains(t, opts.AllowedTools, "finish_autopilot")
