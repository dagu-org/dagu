...
func TestRetrier_Next(t *testing.T) {
	t.Run("SuccessfulRetry", func(t *testing.T) {
		...
		// Second retry - should take ~100ms
		start = time.Now()
		err = retrier.Next(ctx, errors.New("test error"))
		elapsed = time.Since(start)
...
func TestRetrier_WithDifferentPolicies(t *testing.T) {
	t.Run("ConstantBackoff", func(t *testing.T) {
		...
		// Each retry should take the same time
		for i := 0; i < 3; i++ {
			start := time.Now()
			err := retrier.Next(ctx, nil)
			elapsed := time.Since(start)
	...
	t.Run("LinearBackoff", func(t *testing.T) {
		...
		for i, expected := range expectedIntervals {
			...
			require.NoError(t, err, "Retry %d failed", i)
			assert.GreaterOrEqual(t, elapsed, expected)
			assert.Less(t, elapsed, expected+10*time.Millisecond)
		}
