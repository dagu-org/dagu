...
func TestAttempt_Write(t *testing.T) {
	...
	testStatus := createTestStatus(status.StatusRunning)
	...
	assert.Equal(t, status.StatusRunning, actual.Status)
...
func TestAttempt_Read(t *testing.T) {
	...
	status1 := createTestStatus(status.StatusRunning)
	status2 := createTestStatus(status.StatusSuccess)
	...
	assert.NoError(t, err)
	assert.Equal(t, status.StatusSuccess.String(), dagRunStatus.Status.String())

	// Read using ReadStatus
	latestStatus, err := att.ReadStatus(context.Background())
	assert.NoError(t, err)
	assert.Equal(t, status.StatusSuccess.String(), latestStatus.Status.String())
}
...
func TestAttempt_Compact(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if i == 9 {
			...
			testStatus.Status = status.StatusSuccess
		}
	...
	dagRunStatus, err := att.ReadStatus(context.Background())
	assert.NoError(t, err)
	assert.Equal(t, status.StatusSuccess, dagRunStatus.Status)
}
...
func TestAttempt_Close(t *testing.T) {
	...
	// Write some data
	err = att.Write(context.Background(), createTestStatus(status.StatusRunning))
	require.NoError(t, err)
	...
	err = att.Write(context.Background(), createTestStatus(status.StatusSuccess))
	assert.ErrorIs(t, err, ErrStatusFileNotOpen)
...
func TestAttempt_InvalidJSON(t *testing.T) {
	...
	validStatus := createTestStatus(status.StatusRunning)
	...
	dagRunStatus, err := att.ReadStatus(context.Background())
	assert.NoError(t, err)
	assert.Equal(t, status.StatusRunning.String(), dagRunStatus.Status.String())
}
