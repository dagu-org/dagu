...
func TestQueueReader(t *testing.T) {
	...
	err := store.Enqueue(ctx, "test-name", models.QueuePriorityLow, digraph.WorkflowRef{
		Name:       "test-name",
		WorkflowID: "test-workflow-1",
	})
	require.NoError(t, err, "expected no error when adding job to store")

	err = store.Enqueue(ctx, "test-name", models.QueuePriorityHigh, digraph.WorkflowRef{
		Name:       "test-name",
	...
	require.Equal(t, "test-workflow-2", data1.WorkflowID, "expected high priority item first")

	data2 := receivedItems[1].Data()
	require.Equal(t, "test-workflow-1", data2.WorkflowID, "expected low priority item second")
