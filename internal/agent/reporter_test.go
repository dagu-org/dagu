...
func TestReporter(t *testing.T) {
	for scenario, fn := range map[string]func(
		t *testing.T, rp *reporter, mock *mockSender, dag *digraph.DAG, nodes []*history.Node,
	){
...
func testErrorMail(t *testing.T, rp *reporter, mock *mockSender, dag *digraph.DAG, nodes []*history.Node) {
	dag.MailOn.Failure = true
	dag.MailOn.Success = false

	_ = rp.send(context.Background(), dag, history.Status{
...
func testNoErrorMail(t *testing.T, rp *reporter, mock *mockSender, dag *digraph.DAG, nodes []*history.Node) {
	dag.MailOn.Failure = false
	dag.MailOn.Success = true

	err := rp.send(context.Background(), dag, history.Status{
		Status: scheduler.StatusError,
		Nodes:  nodes,
	}, nil)
...
func testSuccessMail(t *testing.T, rp *reporter, mock *mockSender, dag *digraph.DAG, nodes []*history.Node) {
	dag.MailOn.Failure = true
	dag.MailOn.Success = true
...
func testRenderSummary(t *testing.T, _ *reporter, _ *mockSender, dag *digraph.DAG, _ []*history.Node) {
	status := history.NewStatusBuilder(dag).Create("request-id", scheduler.StatusError, 0, time.Now())
...
func testRenderTable(t *testing.T, _ *reporter, _ *mockSender, _ *digraph.DAG, nodes []*history.Node) {
