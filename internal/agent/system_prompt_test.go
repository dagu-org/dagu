...
func TestGenerateSystemPrompt(t *testing.T) {
	...
	t.Run("no memory omits memory section", func(t *testing.T) {
		...
		assert.NotContains(t, result, "<controller_memory")
	...
	t.Run("includes controller memory", func(t *testing.T) {
		t.Parallel()
		env := EnvironmentInfo{DAGsDir: "/dags"}
		mem := MemoryContent{
			GlobalMemory:    "Global info.",
			ControllerMemory: "Controller-specific operating rules.",
		...
		assert.Contains(t, result, "<global_memory>")
		assert.Contains(t, result, `<controller_memory controller="queue_worker">`)
		assert.Contains(t, result, "Controller-specific operating rules.")
	})
	...
	t.Run("controller memory paths appear in output", func(t *testing.T) {
		t.Parallel()
		env := EnvironmentInfo{DAGsDir: "/dags"}
		mem := MemoryContent{
			MemoryDir:     "/dags/memory",
		...
		assert.Contains(t, result, "/dags/memory/MEMORY.md")
		assert.Contains(t, result, "/dags/memory/controller/queue_worker/MEMORY.md")
	})
	...
	t.Run("memory management enforces DAG-first policy", func(t *testing.T) {
		...
		assert.Contains(t, result, "If Controller context is available, save memory to Per-Controller by default (not Global)")
		...
		assert.Contains(t, result, "After updating a Controller's long-lived behavior, operating rules, or learned procedures, create/update that Controller's memory file")
	...
	t.Run("memory management requires confirmation before global write without DAG context", func(t *testing.T) {
		...
		assert.Contains(t, result, "If no DAG or Controller context is available, ask the user before writing to Global memory")
	})
