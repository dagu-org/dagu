...
`)
	...
	child1Rec, err := th.RunStore.FindByChildRequestID(ctx, child1Node.Children[0].RequestID, rootDAG)
	require.NoError(t, err)
	...
	child2Rec, err := th.RunStore.FindByChildRequestID(ctx, child2Node.Children[0].RequestID, rootDAG)
	require.NoError(t, err)
