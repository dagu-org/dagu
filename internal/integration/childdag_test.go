...
`)
	...
	child1Node.Status = status.NodeStatusError
	...
	child2Node.Status = status.NodeStatusError
	...
	require.Equal(t, status.NodeStatusSuccess.String(), child2Status.Status.String())

	// (5) Update the step in child_2 to "failed" to simulate a retry
	child2Status.Nodes[0].Status = status.NodeStatusError
	updateStatus(child2Attempt, child2Status)

	// (6) Check if the child_2 status is now "failed"
	child2Status, err = child2Attempt.ReadStatus(ctx)
	require.NoError(t, err)
	require.Equal(t, status.NodeStatusError.String(), child2Status.Nodes[0].Status.String())
	...
	child2Status, err = child2Attempt.ReadStatus(ctx)
	require.NoError(t, err)
	require.Equal(t, status.NodeStatusSuccess.String(), child2Status.Nodes[0].Status.String())
...
`)
	...
	parentStatus, err := parentAttempt.ReadStatus(ctx)
	require.NoError(t, err)
	require.Equal(t, status.NodeStatusSuccess.String(), parentStatus.Status.String())

	// Find child DAG run
	childNode := parentStatus.Nodes[0]
	require.Equal(t, status.NodeStatusSuccess.String(), childNode.Status.String())
	...
	require.NoError(t, err)
	require.Equal(t, status.NodeStatusSuccess.String(), childStatus.Status.String())

	// Verify the step in child DAG completed successfully after retry
	retryStep := childStatus.Nodes[0]
	require.Equal(t, status.NodeStatusSuccess.String(), retryStep.Status.String())
...
`)
	...
	require.Equal(t, status.NodeStatusSuccess.String(), basicStep.Status.String())
...
`)
	...
	require.NoError(t, err)
	require.Equal(t, status.NodeStatusSuccess.String(), dagRunStatus.Status.String())
...
`)
	...
	require.Equal(t, status.NodeStatusSuccess.String(), successStep.Status.String())
