...
`)
	...
	t.Run("ErrorMailConfigWithEnvVars", func(t *testing.T) {
		t.Parallel()

		th := test.Setup(t)

		// Test that errorMail config with env vars is properly evaluated
		dag := th.DAG(t, `
env:
...
steps:
  - name: test-step
    command: echo "ErrorMail config evaluated successfully"
    output: RESULT
`)
		agent := dag.Agent()
		agent.RunSuccess(t)
		...
		dag.AssertOutputs(t, map[string]any{
			"RESULT": "ErrorMail config evaluated successfully",
		})

		// Verify the errorMail config was evaluated
		require.NotNil(t, dag.DAG.ErrorMail)
		require.Equal(t, "noreply@example.com", dag.DAG.ErrorMail.From)
		require.Equal(t, []string{"admin@example.com"}, dag.DAG.ErrorMail.To)
	})

	t.Run("InfoMailConfigWithSecrets", func(t *testing.T) {
		t.Parallel()

		th := test.Setup(t)

		// Create secret for email address
		secretEmail := "secret-admin@example.com"
		secretFile := th.TempFile(t, "admin-email.txt", []byte(secretEmail))
...
infoMail:
  ...
  to: ${SECRET_ADMIN_EMAIL}
  prefix: "[Info]"

steps:
  - name: test-step
    command: echo "InfoMail config evaluated successfully"
    output: RESULT
`)
		agent := dag.Agent()
		agent.RunSuccess(t)
		...
		dag.AssertOutputs(t, map[string]any{
			"RESULT": "InfoMail config evaluated successfully",
		})

		// Verify the infoMail config was evaluated
		require.NotNil(t, dag.DAG.InfoMail)
		require.Equal(t, "sender@example.com", dag.DAG.InfoMail.From)
		require.Equal(t, []string{secretEmail}, dag.DAG.InfoMail.To)
	})
