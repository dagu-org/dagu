...
func TestNewAPIKey(t *testing.T) {
	before := time.Now().UTC()
	key, err := NewAPIKey("test-key", "Test description", RoleManager, "hash123", "boltbase_tes", "creator-id")
	after := time.Now().UTC()
	...
	assert.Equal(t, "boltbase_tes", key.KeyPrefix)
