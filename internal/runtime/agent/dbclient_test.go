...
func (m *mockDAGStore) SearchPaginated(ctx context.Context, opts exec.SearchDAGsOptions) (*exec.PaginatedResult[exec.SearchDAGResult], []string, error) {
	args := m.Called(ctx, opts)
	if args.Get(0) == nil {
		return nil, args.Get(1).([]string), args.Error(2)
	}
	return args.Get(0).(*exec.PaginatedResult[exec.SearchDAGResult]), args.Get(1).([]string), args.Error(2)
}

func (m *mockDAGStore) SearchMatches(ctx context.Context, fileName string, opts exec.SearchDAGMatchesOptions) (*exec.PaginatedResult[*exec.Match], error) {
	args := m.Called(ctx, fileName, opts)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*exec.PaginatedResult[*exec.Match]), args.Error(1)
}
