...
//     ${VAR} natively, so Boltbase disables its own env expansion to avoid
//     double-expanding values.
//   - fish: intentionally excluded from IsUnixLikeShell (it lacks -e flag
//     support and uses $VAR but not ${VAR}), so Boltbase performs ${VAR} expansion.
//   - Non-Unix (PowerShell, cmd.exe): do not understand ${VAR} syntax at all,
//     so Boltbase must expand variables on their behalf (ExpandEnv stays enabled).
//   - direct / empty: no shell is involved; Boltbase expands OS variables itself.
