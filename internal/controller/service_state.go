...
func (s *Service) controllerWorkingDir(name string) (string, error) {
...
func (s *Service) ensureControllerWorkingDir(name string) (string, error) {
	...
	if err := os.MkdirAll(dir, dirPerm); err != nil {
		return "", fmt.Errorf("create controller workspace: %w", err)
	}
	return dir, nil
}
...
func (s *Service) resetRuntimeForSpecChange(ctx context.Context, name string) error {
	...
	if (state.State == StateRunning || state.State == StateWaiting) &&
		...
		queueTurnMessage(state, "config_updated",
			"Controller configuration changed. Continue with the latest configuration and current instruction.",
			s.clock(),
		)
...
func (s *Service) cancelControllerSession(ctx context.Context, name, sessionID string) error {
...
func (s *Service) reassignSessionUser(ctx context.Context, sessionID, newName string) error {
	...
	return errors.New("session store does not support controller rename with transcript preservation")
}

func (s *Service) assertControllerTargetAvailable(name string) error {
	if pathExists(filepath.Join(s.definitionsDir, name+".yaml")) {
