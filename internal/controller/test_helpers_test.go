...
type testControllerEventStore struct {
	events []*eventstore.Event
}

func (s *testControllerEventStore) Emit(_ context.Context, event *eventstore.Event) error {
	if event == nil {
		return nil
	}
...
func (*testControllerEventStore) Query(context.Context, eventstore.QueryFilter) (*eventstore.QueryResult, error) {
...
func newTestServiceWithEventStore(t *testing.T) (*Service, time.Time, *testControllerEventStore) {
	t.Helper()
	...
	require.NoError(t, err)
	fixedTime := time.Date(2026, time.March, 26, 10, 0, 0, 0, time.UTC)
	store := &testControllerEventStore{}
	svc := New(
		cfg,
		filedag.New(dagsDir, filedag.WithSkipExamples(true)),
		filedagrun.New(runsDir),
