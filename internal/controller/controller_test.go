...
`))
	...
	require.Equal(t, eventstore.TypeControllerError, store.events[0].Type)
}

func TestRuntimeOptionsUsesControllerSoulDocument(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, "controller:software_dev", opts.Soul.ID)
