...
// AutoMigrate performs automatic migration if needed (called from server/scheduler)
func AutoMigrate(ctx *Context) error {
	// Create DAG store
	dagStore := filedag.New(
		ctx.Config.Paths.DAGsDir,
		filedag.WithFlagsBaseDir(ctx.Config.Paths.SuspendFlagsDir),
	)
	...
	needsMigration, err := migrator.NeedsMigration(ctx.Context)
	if err != nil {
		logger.Warn(ctx.Context, "Failed to check for legacy data", "error", err)
	...
	logger.Info(ctx.Context, "Legacy history data detected, starting automatic migration")
	
	// Run migration
	result, err := migrator.Migrate(ctx.Context)
	if err != nil {
		logger.Error(ctx.Context, "Auto-migration failed", "error", err)
		return fmt.Errorf("auto-migration failed: %w", err)
	}
	
	logger.Info(ctx.Context, "Auto-migration completed",
		"migrated", result.MigratedRuns,
		"failed", result.FailedRuns,
	)
	
	// Move legacy data if migration was successful
	if result.FailedRuns == 0 {
		...
		logger.Warn(ctx.Context, "Legacy data not moved due to migration errors", "failed_runs", result.FailedRuns)
	}
