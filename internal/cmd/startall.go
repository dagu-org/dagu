...
func runStartAll(ctx *Context, _ []string) error {
	...
	if ctx.Config.Telegram.Token != "" {
		tgBot, err = initTelegramBot(serviceCtx, ctx)
		if err != nil {
...
// initTelegramBot creates and configures a Telegram bot with its own agent API.
func initTelegramBot(serviceCtx *Context, ctx *Context) (*telegram.Bot, error) {
	stores := ctx.agentStores()
	if stores.ConfigStore == nil {
		return nil, fmt.Errorf("agent config store not available")
	}
	...
	if err != nil {
		logger.Warn(serviceCtx, "Telegram: session store unavailable", tag.Error(err))
	}
	...
	agentAPI.StartCleanup(serviceCtx)
