...
func TestScheduler(t *testing.T) {
	...
	t.Run("Restart", func(t *testing.T) {
		...
		sc, err := scheduler.New(th.Config, entryReader, th.DAGRunMgr, th.DAGRunStore, th.QueueStore, th.ProcStore, th.ServiceRegistry, th.CoordinatorCli, nil, nil)
	...
	t.Run("NextTick", func(t *testing.T) {
		...
		th := test.SetupScheduler(t)
		schedulerInstance, err := scheduler.New(th.Config, newMockJobManager(), th.DAGRunMgr, th.DAGRunStore, th.QueueStore, th.ProcStore, th.ServiceRegistry, th.CoordinatorCli, nil, nil)
		require.NoError(t, err)
...
func TestFileLockPreventsMultipleInstances(t *testing.T) {
	...
	sc1, err := scheduler.New(th.Config, entryReader, th.DAGRunMgr, th.DAGRunStore, th.QueueStore, th.ProcStore, th.ServiceRegistry, th.CoordinatorCli, nil, nil)
	require.NoError(t, err)
	sc1.SetClock(clock)
	...
	sc2, err := scheduler.New(th.Config, newMockJobManager(), th.DAGRunMgr, th.DAGRunStore, th.QueueStore, th.ProcStore, th.ServiceRegistry, th.CoordinatorCli, nil, nil)
	require.NoError(t, err)
	sc2.SetClock(clock)
