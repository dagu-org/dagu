...
class DaguClient:
    ...
    def start_dag(self, dag_name, params=None):
        url = f"{self.base_url}/api/v2/dags/{dag_name}/start"
        data = {}
        if params:
    ...
    def get_dag_run_status(self, dag_name, run_id):
        url = f"{self.base_url}/api/v2/dag-runs/{dag_name}/{run_id}"
        response = requests.get(url, headers=self.headers)
        return response.json()

# Usage
client = DaguClient("http://localhost:8080", token="your-token")
result = client.start_dag("etl-pipeline.yaml", {"env": "production", "date": "2024-02-11"})
...
func (c *DaguClient) StartDAG(dagName string, params map[string]interface{}) (*StartDAGResponse, error) {
    paramsJSON, _ := json.Marshal(params)
    reqBody := StartDAGRequest{
        Params: string(paramsJSON),
    }
    
    body, _ := json.Marshal(reqBody)
    req, err := http.NewRequest("POST", 
        fmt.Sprintf("%s/api/v2/dags/%s/start", c.BaseURL, dagName),
...
func main() {
    ...
    result, err := client.StartDAG("data-processing-pipeline", params)
    if err != nil {
        panic(err)
    }
    
    fmt.Printf("Started DAG run: %s\n", result.DAGRunID)
}
```
...
class DaguAPI {
  ...
  async startDAG(fileName, params = {}, dagRunId = null) {
    const response = await this.client.post(`/dags/${fileName}/start`, {
  ...
  async getDAGRunStatus(name, dagRunId) {
    const response = await this.client.get(`/dag-runs/${name}/${dagRunId}`);
    return response.data;
  }
...
// Usage
const dagu = new DaguAPI('http://localhost:8080', 'your-token');
(async () => {
  try {
    // Start a DAG
    const run = await dagu.startDAG('data-processing-pipeline', {
      date: '2024-02-11',
      env: 'production',
      batch_size: 5000
    });
    console.log('Started DAG run:', run.dagRunId);
    
    // Check status
    const status = await dagu.getDAGRunStatus('data-processing-pipeline', run.dagRunId);
    console.log('Current status:', status.dagRunDetails.statusLabel);
    
    // Get logs
    const logs = await dagu.getDAGRunLog('data-processing-pipeline', run.dagRunId, {tail: 100});
    console.log('Recent logs:', logs.content);
  } catch (error) {
