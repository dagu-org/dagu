...
function DAGStepTableRow({ step, index }: Props) {
  ...
  const preconditions = step.preconditions?.map((c, index) => (
    <div
      key={index}
      className="flex items-center gap-2 mb-1 text-xs bg-slate-100 dark:bg-slate-800 rounded-md p-2"
    >
  ...
  return (
    <TableRow className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-200">
      {/* Number */}
      ...
      <TableCell>
        <div className="space-y-2">
          <div className="text-base font-semibold text-slate-800 dark:text-slate-200 break-all">
            {step.name}
          </div>
          {step.description && (
          ...
          {step.run && (
            <div className="mt-1 flex w-fit items-center gap-2 rounded-md bg-purple-50 px-2 py-1 text-xs dark:bg-purple-900/20">
      ...
      <TableCell>
        <div className="space-y-2.5">
          ...
          {/* Script */}
          {step.script && (
            <div className="flex items-center gap-2 text-xs bg-amber-50 dark:bg-amber-900/10 rounded-md px-2 py-1 w-fit">
          ...
          {/* Directory */}
          {step.dir && (
            <div className="flex items-center gap-2 text-xs bg-slate-50 dark:bg-slate-800/50 rounded-md px-2 py-1 w-fit">
          ...
          {/* Output */}
          {step.output && (
            <div className="flex items-center gap-2 text-xs bg-green-50 dark:bg-green-900/10 rounded-md px-2 py-1 w-fit">
          ...
          <div className="flex flex-wrap gap-1">
            {step.depends.map((dep, idx) => (
              <Badge
                key={idx}
                variant="outline"
                className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-1 text-wrap break-all"
              >
                {dep}
              </Badge>
            ))}
      ...
      <TableCell>
        <div className="space-y-2">
          ...
          {step.repeatPolicy?.repeat && (
            <Badge
              variant="outline"
              className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800 px-2.5 py-1"
            >
          ...
          {/* Mail on Error */}
          {step.mailOnError && (
            <div className="flex items-center gap-2 text-xs bg-red-50 dark:bg-red-900/10 rounded-md px-2 py-1 w-fit">
          ...
          {(step.stdout || step.stderr) && (
            <div className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-md p-2">
              {step.stdout && (
                <div className="mb-1">
