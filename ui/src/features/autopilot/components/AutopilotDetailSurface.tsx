...
import { DAGNamePicker } from '@/features/autopilot/components/AutopilotCreateModal';
import { AutopilotDocumentSection } from '@/features/autopilot/components/AutopilotMemorySection';
import type { AutopilotDetailController } from '@/features/autopilot/hooks/useAutopilotDetail';
import {
  agentMessageLabel,
  autopilotDisplayName,
...
function autopilotControllerMessage(
  status?: components['schemas']['AutopilotControllerStatus']
): string | undefined {
  if (!status) {
  ...
  switch (status.state) {
    case 'ready':
      return 'Autopilot controller is ready.';
    case 'disabled':
      return 'Autopilot is disabled in agent settings.';
    case 'unavailable':
      return 'No active scheduler with a ready Autopilot controller is available.';
    default:
      return 'Scheduler controller readiness is unknown.';
  }
}
...
function RunRow({
  ...
  run: AutopilotRunSummary & { isCurrent?: boolean };
  onOpen: (run: AutopilotRunSummary) => void;
}): React.ReactElement {
  return (
    <button
      type="button"
...
function ThreadBubble({
  ...
  item: AutopilotDetailController['threadItems'][number];
}): React.ReactElement {
  if (item.kind === 'queued') {
    return (
...
function TalkThread({
  ...
  items: AutopilotDetailController['threadItems'];
  sessionId?: string;
  active: boolean;
}): React.ReactElement {
...
function TaskList({
  ...
  onEditTask: (task: AutopilotTaskTemplate) => void | Promise<void>;
  ...
  onDeleteTask: (task: AutopilotTaskTemplate) => void | Promise<void>;
  disabled: boolean;
  ...
  return (
    <div className="min-w-0 rounded-lg border p-4">
      ...
      {items.length ? (
        ...
        <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
          No current cycle is active. Start the autopilot or wait for the next
          scheduled cycle.
        </div>
      )}
    </div>
...
function StatusTab({
  ...
  controller: AutopilotDetailController;
  active: boolean;
  onOpenRun: (run: AutopilotRunSummary) => void;
}): React.ReactElement {
          ...
          <div className="space-y-2 text-sm text-muted-foreground">
            <p>
              {controller.scheduleConfigured
                ...
                : 'No schedule is configured for this autopilot.'}
            </p>
            {controller.detail?.state.pendingPrompt ? (
              <p>
                Human input is currently required before the Autopilot can
                continue.
              </p>
            ) : controller.canSendOperatorMessage ? (
              <p>
          ...
          <div className="space-y-2">
            {controller.lifecycleState === 'paused' ? (
              <div className="rounded-md border border-slate-300/60 bg-slate-100/70 px-3 py-2 text-sm text-slate-800 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-200">
                Response will be queued, and the Autopilot will remain paused
                until you resume it.
              </div>
            ) : null}
            ...
            placeholder={
              startMode
                ? hasStandingInstruction
                  ...
                  : 'Tell this Autopilot what task to work on.'
                ...
                : controller.detail?.state.pendingPrompt
                  ...
                  : controller.canSendOperatorMessage
                    ? controller.detail?.state.state === 'paused'
                      ? 'This records your message now, but the Autopilot will stay paused until you resume it.'
                      : controller.detail?.state.currentRunRef
                        ? 'This records your message now and the Autopilot will pick it up after the current child DAG changes state.'
                        : 'This queues a user message into the active Autopilot task.'
                    : 'Operator messages are only accepted while the Autopilot has an active task.'}
            </div>
            <Button
              onClick={() => {
                if (startMode) {
...
function ConfigTab({
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="min-w-0 rounded-lg border p-4">
          ...
          <div className="space-y-3 text-sm">
            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-goal">Goal</Label>
              <Textarea
                id="autopilot-detail-goal"
              ...
              />
              <div className="text-xs text-muted-foreground">
                Optional. Leave blank if this Autopilot should work from the
                instruction, task list, and runtime context.
              </div>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-standing-instruction">
                Standing Instruction
              </Label>
              <Textarea
                id="autopilot-detail-standing-instruction"
            ...
            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-schedule">Schedule</Label>
              <Textarea
                id="autopilot-detail-schedule"
            ...
            <div className="flex items-center justify-between gap-4 rounded-md border px-3 py-2">
              <Label htmlFor="autopilot-detail-reset-on-finish">
                Reset on finish
              </Label>
              <Switch
                id="autopilot-detail-reset-on-finish"
            ...
            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-icon-url">Image URL</Label>
              <Input
                id="autopilot-detail-icon-url"
              ...
              <div className="text-xs text-muted-foreground">
                ...
                like /assets/autopilot/build-captain.png.
              </div>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-model">Model</Label>
              <Select
                ...
                disabled={metadataFieldDisabled}
              >
                <SelectTrigger id="autopilot-detail-model">
              ...
              <div className="text-xs text-muted-foreground">
                Set an Autopilot-specific model here instead of inheriting the
                global default used by fresh sessions.
              </div>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="autopilot-detail-description">Description</Label>
              <Input
                id="autopilot-detail-description"
            ...
            <div className="space-y-3 border-t pt-3">
              <div className="text-xs text-muted-foreground">
                {controller.isEditingSpec
                  ...
                  : 'This updates the top-level metadata fields in the Autopilot spec.'}
              </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <AutopilotDocumentSection
          autopilotName={controller.detail?.definition.name || ''}
          document={AutopilotDocument.MEMORY_md}
        />
        <AutopilotDocumentSection
          autopilotName={controller.detail?.definition.name || ''}
          document={AutopilotDocument.SOUL_md}
        />
      </div>
...
export function AutopilotDetailSurface({
  controller,
  headerCaption,
  renderHeaderActions,
}: {
  ...
  const [selectedRun, setSelectedRun] =
    React.useState<AutopilotRunSummary | null>(null);
    ...
    return (
      <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
        {controller.loadError instanceof Error
          ...
          : 'Failed to load Autopilot details'}
      </div>
    );
  }
  ...
  if (!controller.detail) {
    return (
      <div className="p-8 text-sm text-muted-foreground">
        Autopilot definition not found.
      </div>
    );
  }

  const runtimeControllerMessage = autopilotControllerMessage(
          ...
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <h1 className="truncate text-2xl font-semibold">
                {autopilotDisplayName(controller.detail.definition)}
              </h1>
